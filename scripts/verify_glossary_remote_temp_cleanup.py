...
if not remote_match:
    ...
    if "if let httpResponse = response as? HTTPURLResponse" in body:
        errors.append("importRemote must not receive HTTP error placeholder files; download() should throw HTTP failures before returning")
    download_idx = body.find("let (temporaryURL, _) = try await download(url: url, progress: progress)")
    temp_guard_idx = body.find("var downloadedURL: URL? = temporaryURL", download_idx)
    ...
    if -1 in [download_idx, temp_guard_idx, defer_idx, size_idx, move_idx, clear_idx, parse_idx] or not (download_idx < temp_guard_idx < defer_idx < size_idx < move_idx < clear_idx < parse_idx):
        errors.append("importRemote must install temp cleanup immediately after a successful download, keep it active through file-size/move failures, and clear it only after a successful move")

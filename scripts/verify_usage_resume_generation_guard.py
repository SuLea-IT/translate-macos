...
if "private var pendingUsageResumeReplayChunks: [BufferedAudioChunk] = []" not in text:
    errors.append("AppState must keep pending replay chunks for an in-flight usage resume task")
...
if not schedule_match:
    ...
    if "resumeFromUsagePause(replayChunks: replayChunks, generation: generation)" in body:
...
for name, pattern in [
    ...
    if "pendingUsageResumeReplayChunks.removeAll" not in body:
